...
public:
	...
	void stepHighway(double egoVelocity, long long timestamp, int frame_per_sec, pcl::visualization::PCLVisualizer::Ptr& viewer, ofstream& out_file_)
	{
		...
		for (int i = 0; i < traffic.size(); i++)
		{
			...
			if(trackCars[i])
			{
				...
				tools.lidarSense(traffic[i], viewer, timestamp, visualize_lidar, out_file_);
				tools.radarSense(traffic[i], egoCar, viewer, timestamp, visualize_radar, out_file_);
